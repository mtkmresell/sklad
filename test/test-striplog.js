// Test: jednorázový úklid zbytků po evidenci přesunů profilů
...
(async () => {
  ...
  await ctx.addInitScript((savedAt) => {
    localStorage.setItem('sklad_v3', JSON.stringify([
      { id:'p1', name:'S evidencí přesunů', category:'sneakers', personal:true, buyPrice:4000, buyCurrency:'CZK',
        saleState:'stock', location:'Doma', dateAdded:Date.now(), buyDate:'2023-09-15', tags:[],
  ...
  const again = await page.evaluate(() => stripProfileLogs());
